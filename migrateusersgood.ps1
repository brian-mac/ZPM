<# 
.synopsis 
 This script will: Add the specified user/s to the Google MigratedGapp group,and set forwarding to a specified value.
Version 4.0.  It will also detect if a the source emailaddress relates to a conteact and convert it to a user.
...
.Example
To call this script in Windows 10 with an input file  : powershell .\MigrateADT2PropertiesV03.ps1 -inputfile 'C:\temp\MigratedUsersT2.csv'

.Example    
To call this script in windows 10 to migrate a user DL: powershell .\MigrateADT2PropertiesV03.ps1 -Email 'Brian.mcelhinney@allegisgroup' -O365ForwardingAddress 'Brian.mcelhinney@csfb.com'
#>
...
param (
    ...
    [Parameter(Mandatory=$True,HelpMessage="Please enter Forwarding Email Address",Parametersetname = "Set 2" )][string] $AGSEmail
)
...
function ConnectToTalent2Asia ()
{
    If ( $ADsession -eq $Null -or $ADSession.State -eq "Closed")
    {
        ...
        Catch
        {
            WriteLine " : Could not create session to Talent2.corp.  Incorrect users details?"
        }
    }
}
...
function AddSigniture ($targetUser,$AGSEmail)
{
    ...
    Switch -wildcard ($TargetOffice) 
    {
        ...
        Default  {$SignitureGroup = "Exclaimer-APAC-Internal"
                  $GroupSuffix = "Std"}
    }
    IF ($AGSEmail)
    {
    ...
    AddToDlist $Targemail $SignitureGroup 
    $GroupSuffix  = $null
}
...
Function Remove-T2Contact ($TargetContact , $TargetRecipent)
{
    ...
    {
        $Writeline = "Could not find a contact $TContact  Unexpected this was"
        WriteLine $Writeline
    }
    try 
    {
        remove-OnPremMailContact -identity $T2Contact.alias -Confirm:$false
        $Line = "Removed the contact $T2Contact for $TargetUser"
        WriteLine $Line   
    }
    Catch 
    {
        $Line = "Could not remove the contact $T2Contact for $TargetUser"
        WriteLine $Line     
    }
    try 
    {
        Set-OnPremRemoteMailbox -identity $TargetRecipent -emailAddresses  @{Add=$T2CEx500}  
        $Writeline = "$T2CEx500 proxy address added to $TargetRecipent"  
        WriteLine $Writeline
    }
    catch 
    {
        $Writeline = "$T2CEx500 proxy address could not be added to $TargetRecipent"  
        WriteLine $Writeline
    }
}
Function ChangeForwarding ($TargetUser, $ForwardingAddress, $AGSEmail)
{
    ...
    If ($TestUser)
    {
        ...
        $Line = "$TargetUser  O365 forwarding has been set to $ForwardingAddress."
        WriteLine $Line
        If ($T2Conact)
        {
    ...
    Else 
    {
        $Line = "$TargetUser  Error O365  forwarding has Not been set to $Forwardingaddress."
        WriteLine $Line
    }
    $T2Conact = $Null
    $TargetIdentity = $Null
...
Function ConvertUser($TargetUser)
{
    ...
    Try
    {
       #Create the new user with basic properties.
        ...
        -name $UserName
        $Line = "Sucsesfuly create user $TargetUser with an SAM of $NewSam"
        WriteLine $ErrorLine
    }
    Catch
    {
        $Line = "Could not create user $TargetUser with an SAM of $NewSam"
       WriteLine $ErrorLine
    } 
    try 
    {
        ...
        $Line = "Set defined properties of $NewSam"
        WriteLine $ErrorLine
   }
   catch
   {
        $Line = "Could not set all properties of user $TargetUser with an SAM of $NewSam"
        WriteLine $ErrorLine
   }
   try 
   {
        ...
        $Line = "Set all Group Memebrship of user $TargetUser with an SAM of $NewSam"
        WriteLine $ErrorLine    
    }
    catch
    {
        $Line = "Could not set all group membership of user $TargetUser with an SAM of $NewSam"
        WriteLine $ErrorLine
    }
    Try
    {
        ...
        $Line = "Password Set for $NewSam and account enabled"
        WriteLine $ErrorLine
    }
    Catch
    {
        $Line = "Could not set password for $NewSam and account not Enabled"
        WriteLine $ErrorLine
    }
...
Function ProcessUser($MigratedUser, $ForwardingAddress, $AGSEmail)
{
    ...
    Try 
    {
        ...
        {
            $Error = "$targetuser is a Contact, attempting to convert to user"
    ...
    Catch 
    {
        $Error = $MigratedUser + " Does not exist, please check Email Address"
        WriteLine $Error
    }
    ...
    If ($ForwardingAddress)
    {
        try 
        {
            ...
            $Line = "$TargetUser  extensionAttribute1 flag has been set $ForwardingAddress"
            WriteLine $Line
        }
        catch 
        {
            $Line = "$TargeUser extensionAttribute1 could not be set to False"
            WriteLine $Line
        }    
    }
    else
     {
        try 
        {
            ...
            $Line = "$TargetUser  extensionAttribute2 flag has been set to False"
            WriteLine $Line
        }   
        catch 
        {
            $Line = "$TargeUser extensionAttribute2 could not be set to False"
            WriteLine $Line
        }
     }       
...
if ($inputfile.Length -ne 0)
{
    ...
    {
        $ForwardingAddress = $MigratedUser.ForwardingAddress 
        $O365User = $MigratedUser.email
        $AGSEmail = $MigratedUser.AGSEmail
