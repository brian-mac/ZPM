...
#>
$LogFile = CreateFile "C:\temp\RoomListLog.txt"  "Append"
$LogStream = New-Object System.IO.StreamWriter($LogFile)
$DlOu = "OU=Internal Only,OU=Distribution List,OU=APAC,OU=Groups,OU=Enterprise,DC=allegisgroup,DC=com"


$Rooms = Import-Csv -Path $RoomFile
$ListFlag = $False
$FirstList =$True
foreach ($Room in $Rooms)
{
    ...
    if ($FirstList -or $ListFlag) 
    {
        $RoomSMTP = $Roomlist + "@Allegisgroup.com"
        New-OnPremDistributionGroup -Name $RoomList –PrimarySmtpAddress $RoomSMTP -OrganizationalUnit $DlOu  –RoomList
        $FirstList = $False
    }
    If (!$ListFlag)
    {
         Add-OnPremDistributionGroupMember -Identity $RoomList -Member $RoomName
    }
    $Lastlist = $RoomList
