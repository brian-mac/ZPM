...
Function ConvertUser($TargetUser)
{
   ...
   {
        $Line = "Error: Could not set all properties of user $TargetUser with an SAM of $NewSam"
        WriteLine $Line
   }
   try 
   {
...
Function ProcessUser($MigratedUser, $ForwardingAddress, $AGSEmail)
{
    ...
    Try 
    {
        ...
        if ($Cast.basetype.name -eq "ADentity")
        {
            ...
            {
                $Error = "Error: $targetuser is a Contact, attempting to convert to user"
                Writeline $Error
    ...
    If ($ForwardingAddress)
    {
        try 
        {
            ...
            $Line = "Sucsess: $TargetUser  extensionAttribute1 flag has been set $True"
            WriteLine $Line
        }
        catch 
        {
